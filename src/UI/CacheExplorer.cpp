...
std::pair<TBStyle, TBStyle> CacheExplorer::DrawFn(std::size_t i, Vec2i pos, int w, bool hovered, Char trailing)
{
	...
	// Update
	const String updated = fmt::format(Settings::Layout::Cache::date_format, m_it->second.updated.time_since_epoch());
	w2 -= Draw::TextLine(updated, {Settings::Style::Cache::update.fg, s.bg, Settings::Style::Cache::update.s},
			pos+Vec2i(w2-Util::SizeWide(updated), 0), w2-p, trailChar).first;
	...
	return {s, {Settings::Style::Cache::numbers.fg, s.bg, Settings::Style::Cache::numbers.s}};
}
